...
struct List_s{
    ...
    Node* head;
    Node* tail;
    // Current node pointer should keep track of the last item modified in the list
    Node* current;
};
...
typedef struct NodePool NodeContainer;
struct NodePool{
    int usedHeads;
    int usedNodes;
    List availHeads[LIST_MAX_NUM_HEADS];
    Node availNodes[LIST_MAX_NUM_NODES];
};
